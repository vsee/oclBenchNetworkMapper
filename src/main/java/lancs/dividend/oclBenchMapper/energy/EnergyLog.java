...
public class EnergyLog implements Serializable {
	...
	private void calculateStatistics(List<List<String>> records) {
		...
		for (List<String> rec : records) {
			energyJ += Double.parseDouble(rec.get(OclBenchMapperCsvHandler.FIELD_ELOG_ENERGY_IDX));
			runtimeMS += (Double.parseDouble(rec.get(OclBenchMapperCsvHandler.FIELD_ELOG_STOP_IDX)) - 
					Double.parseDouble(rec.get(OclBenchMapperCsvHandler.FIELD_ELOG_ISSUE_IDX))) / 1000000;
		}
	}
}
